...
export function buildShippingSlipHtml(order: AdminOrderRow): string {
  ...
  const bodyRows = items
    .map((raw) => {
      ...
      const lineTot = esc(formatNaira(unit * q))
      return `<tr><td>${name}</td><td>${vl}</td><td class="num">${q}</td><td class="num">${lineTot}</td></tr>`
    })
    .filter(Boolean)
    .join('')
...
<head>
  ...
  <title>TLE-BRAND — Shipping slip #${tag}</title>
  <style>
...
<body>
  <div class="sheet">
    ...
    <div class="inner">
      ...
      <table class="items">
        <thead><tr><th>Item</th><th>Finish / notes</th><th class="num">Qty</th><th class="num">Line</th></tr></thead>
        <tbody>${bodyRows || '<tr><td colspan="4" class="muted">No line items.</td></tr>'}</tbody>
      </table>
      ...
      <p class="foot">Internal document — not a tax invoice unless issued separately with VAT details.<br/>Attach to parcel or include in shipment.</p>
    </div>
  </div>
</body>
</html>`
